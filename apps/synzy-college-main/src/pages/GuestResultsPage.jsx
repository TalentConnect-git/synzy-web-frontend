...
import { ArrowLeft, college, MapPin, Users, Clock, DollarSign, Star, ExternalLink } from 'lucide-react';
...
const GuestResultsPage = () => {
  ...
  useEffect(() => {
    ...
    // Simulate API call to get matching colleges
  ...
  const fetchMatchingcolleges = async () => {
    ...
    try {
      ...
      const params = {
        ...
        collegeMode: criteria.collegeMode ? [criteria.collegeMode] : (prefs.collegeType ? [prefs.collegeType] : []),
      ...
      const result = await searchcolleges(params);
      const colleges = Array.isArray(result?.data) ? result.data : [];
      setMatchingcolleges(colleges);
    } catch (error) {
      console.error('Error fetching colleges:', error);
      toast.error('Failed to fetch matching colleges');
      setMatchingcolleges([]);
    } finally {
      setIsLoading(false);
    }
    ...
    <div className="bg-white rounded-xl shadow-lg p-6 hover:shadow-xl transition-shadow">
      <div className="flex flex-col md:flex-row gap-4">
        <div className="md:w-1/3">
          <img 
            src={
              (college.photos && college.photos[0]?.url) ||
              college.image ||
        ...
        <div className="md:w-2/3">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-4">
            <div className="flex items-center gap-2">
              <college className="w-4 h-4 text-blue-600" />
              <span className="text-sm text-gray-600">{college.board}</span>
            </div>
            <div className="flex items-center gap-2">
              <MapPin className="w-4 h-4 text-green-600" />
              <span className="text-sm text-gray-600">{college.area || college.city || college.state}</span>
            </div>
            <div className="flex items-center gap-2">
              <Users className="w-4 h-4 text-purple-600" />
              <span className="text-sm text-gray-600 capitalize">{college.genderType}</span>
            </div>
            <div className="flex items-center gap-2">
              <DollarSign className="w-4 h-4 text-green-600" />
              <span className="text-sm text-gray-600">{college.feeRange}</span>
            </div>
          </div>
          ...
          <div className="flex justify-between items-center">
            ...
            <button
              onClick={() => handleViewcollege(college._id || college.collegeId)}
      ...
      <div className="min-h-screen bg-gray-50 py-8">
        <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="text-center">
            ...
            <h2 className="text-xl font-semibold text-gray-700">Finding colleges that match your preferences...</h2>
          </div>
        </div>
      </div>
    );
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8">
        ...
        <div className="mb-8">
          ...
          <p className="text-gray-600">
            We found {matchingcolleges.length} colleges that match your criteria
          </p>
        </div>
        ...
        <div className="grid md:grid-cols-3 gap-6 mb-8">
          ...
          <div className="bg-yellow-50 border border-orange-200 rounded-xl p-6">
            <h3 className="text-lg font-semibold text-gray-800 mb-3">
              Which colleges match your preferences?
            </h3>
            <button
          ...
          <div className="bg-yellow-50 border border-orange-200 rounded-xl p-6">
            <h3 className="text-lg font-semibold text-gray-800 mb-3">
              Want the latest insights on colleges?
            </h3>
            <button
        ...
        {showPredictor && (
          <div className="bg-blue-50 border border-blue-200 rounded-xl p-6 mb-8">
            ...
            <p className="text-blue-700 mb-4">
              Based on your preferences, our AI has analyzed {matchingcolleges.length} colleges and recommends the top matches below.
            </p>
            <div className="bg-white rounded-lg p-4">
              <p className="text-gray-700">
                <strong>Analysis:</strong> Your preferences show a strong inclination towards {preferences?.collegeType || 'private'} colleges with {preferences?.boards || 'CBSE'} board. 
                The AI has prioritized colleges that offer {preferences?.interests || 'academic excellence'} and are located in {preferences?.state || 'your preferred state'}.
              </p>
            </div>
          </div>
        )}
        ...
        <div className="space-y-6">
          <h2 className="text-2xl font-bold text-gray-800 mb-6">
            Recommended colleges ({matchingcolleges.length})
          </h2>
          ...
          {matchingcolleges.length > 0 ? (
            ...
            <div className="text-center py-12">
              <college className="w-16 h-16 text-gray-400 mx-auto mb-4" />
              <h3 className="text-xl font-semibold text-gray-600 mb-2">No colleges found</h3>
              <p className="text-gray-500 mb-4">
                We couldn't find any colleges matching your current preferences.
              </p>
              <button
                onClick={handleEditPreferences}
        ...
        <div className="mt-12 text-center bg-white rounded-xl shadow-lg p-8">
          ...
          <p className="text-gray-600 mb-6">
            Create an account to save your preferences and apply to colleges directly.
          </p>
